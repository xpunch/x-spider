...
def convert_weibo_time(time_string):
    now_time = datetime.datetime.now()
    if '分钟前' in time_string:
        minutes = re.search(r'^(\d+)分钟', time_string).group(1)
        created_at = now_time - datetime.timedelta(minutes=int(minutes))
        format_str = created_at.strftime('%Y-%m-%d %H:%M')
        return dateutil.parser.parse(format_str)
    ...
    if '今天' in time_string:
        format_str = time_string.replace('今天', now_time.strftime('%Y-%m-%d'))
        return dateutil.parser.parse(format_str)
...
def extract_weibo_tweet(weibo_html):
    s = weibo_html
    if '转发理由' in s:
...
def extract_weibo_reply(reply_html):
    s = reply_html
    if 'class="ctt">' in s:
        s = s.split('class="ctt">', maxsplit=1)[1]
